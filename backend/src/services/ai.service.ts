...
export async function analyzeReport(
  ...
  try {
    ...
    const prompt = 'Bola: ' + child.full_name + ', ' + age + ' yoshli, tashxis: ' + child.diagnosis +
      '. Bugungi hisobot: Kayfiyat: ' + report.mood_score + '/10. Uyqu: ' +
      (report.sleep_hours || 'kiritilmagan') + ' soat. Ishtaha: ' + (report.appetite || 'kiritilmagan') +
      '. Nutq kuzatuvi: ' + (report.speech_notes || 'kiritilmagan') +
      '. Xulq-atvor: ' + (report.behavior_notes || 'kiritilmagan') +
      '. Vazifalar: ' + (report.tasks_completed || 0) + '% bajarildi. Qo\'shimcha: ' +
      (report.notes ?? "yo'q") + '. ' + previousText +
      'Iltimos, qisqacha tahlil va doktorga 2-3 ta tavsiya ber. Tavsiyalar uchun O\'zbek tilida.';
    const message = await anthropic.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 600,
      temperature: 0.3,
      system: 'Sen NeuroCare platformasining AI yordamchisisans. Bolalar rivojlanish hisobotlarini tahlil qilish va doktorlarga qisqacha, amaliy xulosa berish. Hech qachon rasmiy tashxis qo\'ymay. Faqat kuzatuv va tavsiyalar ber. O\'zbek tilida javob ber. Qisqa va aniq bo\'l.',
      messages: [{ role: 'user', content: prompt }],
    });
    const response = message.content[0].type === 'text' ? message.content[0].text : '';
    return { summary: response, timestamp: new Date() };
  } catch (error) {
...
export async function parentChatbot(
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return { answer: 'Hozircha javob bera olmayapman. Doktoringiz bilan bog\'laning.', timestamp: new Date() };
  }
  try {
    const systemMessage = 'Sen NeuroCare platformasining ota-onalar uchun 24/7 yordamchi chatbotisan. Bolalar rivojlanishi haqida savollarga javob ber. Uy mashqlari va metodikalar haqida maslahat. Ota-onalarni emotsional qo\'llab-quvvatlash. O\'zbek tilida. Rasmiy tashxis yo\'q. Dori yo\'q. Oxirida doktorga maslahatlashishni eslat. Bola: ' + childInfo.full_name + ', ' + childInfo.age + ' yoshli, tashxis: ' + childInfo.diagnosis;
    ...
    const response = message.content[0].type === 'text' ? message.content[0].text : '';
    return { answer: response || 'Xatolik yuz berdi.', timestamp: new Date() };
  } catch (error) {
...
export function adaptGameLevel(
  ...
  if (!recent_sessions || recent_sessions.length === 0) {
    return { new_level: current_level, reason: "Yetarli ma'lumot yo'q. Shu darajada davom eting.", motivational_message: 'Davom et! Ko\'rib chiqaylik keyingi mashqlarda.', avg_score: 0 };
  }
  ...
  const motivationalMessages = [
    ...
    'Bunda davom et! Eng qiziqishi oxirida kutilgan!',
  ...
  return { new_level: newLevel, reason, motivational_message, avg_score: avgScore };
}

// triggerAIAnalysis -> analyzeAndSaveReport (ta'riflangan)
export async function triggerAIAnalysis(
  reportId: string,
  childId: string,
  reportData: any
) {
  return analyzeAndSaveReport(reportId, childId, reportData);
}
...
export async function analyzeAndSaveReport(
  ...
  try {
    ...
    await supabaseAdmin.from('reports').update({ ai_summary: result.summary }).eq('id', reportId);
    return { success: true };
  } catch (error) {
